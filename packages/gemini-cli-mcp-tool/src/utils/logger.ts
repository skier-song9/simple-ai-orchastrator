...
export class Logger {
  ...
  static commandExecution(
    ...
    this.debug(
      `Executing command: ${chalk.magenta(command)} ${args.join(" ")} at ${new Date(startTime).toISOString()}`
    );
  }
  ...
  static geminiResponse(response: string, tokensUsed?: number): void {
    ...
    this.debug(
      `Gemini response received${tokens}: ${response.slice(0, 100)}...`
    );
  }

  static sandboxMode(mode: string, command: string): void {
    this.info(
      `Execution mode: ${chalk.yellow(mode)} - executing: ${chalk.cyan(command)}`
    );
  }
