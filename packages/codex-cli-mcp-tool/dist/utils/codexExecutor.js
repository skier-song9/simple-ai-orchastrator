...
export async function executeCodex(prompt, options = {}, onProgress) {
    const { model, sandbox, workingDir, timeout, image, config, profile, fullAuto, } = options;
    // Build command arguments: codex exec [OPTIONS] "prompt"
    const args = [CLI.SUBCOMMANDS.EXEC];
    // Add model selection
    if (model) {
    ...
    args.push(prompt);
    Logger.sandboxMode(sandbox || (fullAuto ? "full-auto" : CLI.DEFAULTS.SANDBOX), `${CLI.COMMANDS.CODEX} ${args.join(" ")}`);
    try {
        const rawOutput = await executeCommand(CLI.COMMANDS.CODEX, args, onProgress, timeout, workingDir);
        const parsedOutput = parseCodexOutput(rawOutput);
        // Attach metadata
        parsedOutput.metadata.model = model || CLI.DEFAULTS.MODEL;
        if (fullAuto)
            parsedOutput.metadata.approvalMode = "full-auto";
        if (sandbox)
            parsedOutput.metadata.sandbox = sandbox;
        if (workingDir)
 ...
 */
export function formatCodexResponseForMCP(output, includeMetadata = true) {
    let formatted = "";
