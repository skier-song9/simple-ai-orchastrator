...
export class Logger {
    ...
    static geminiResponse(response, tokensUsed) {
        const tokens = tokensUsed ? ` (${tokensUsed} tokens)` : "";
        this.debug(`Gemini response received${tokens}: ${response.slice(0, 100)}...`);
    }
    static sandboxMode(mode, command) {
        this.info(`Execution mode: ${chalk.yellow(mode)} - executing: ${chalk.cyan(command)}`);
    }
