...
export function parseGeminiOutput(
  ...
  const metadata: GeminiMetadata = { outputFormat };
  ...
  if (outputFormat === "json" || outputFormat === "stream-json") {
    ...
    try {
      ...
      if (outputFormat === "stream-json" || trimmed.startsWith("{")) {
        ...
        const output: GeminiOutput = { metadata, response: responseText, rawOutput };
  ...
  const output: GeminiOutput = { metadata, response, rawOutput };
 ...
 */
export function formatGeminiResponse(
  output: GeminiOutput,
