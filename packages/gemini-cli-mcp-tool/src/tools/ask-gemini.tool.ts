...
const askGeminiArgsSchema = z.object({
  prompt: z
    ...
    .min(1)
    .describe(
      "User query or instruction for Gemini. Can include file references and complex requests."
    ),
  model: z
    .string()
    .optional()
    .describe(
      `Optional model to use. Options: ${Object.values(MODELS).join(", ")}. Defaults to ${MODELS.GEMINI_2_5_PRO}.`
    ),
  approvalMode: z
    .string()
    .optional()
    .describe(
      `Approval mode: ${Object.values(APPROVAL_MODES).join(", ")}. Defaults to default.`
    ),
  yolo: z
    .boolean()
    .optional()
  ...
  outputFormat: z
    ...
    .optional()
    .describe(
      `Output format: ${Object.values(OUTPUT_FORMATS).join(", ")}. Defaults to text.`
    ),
  workingDir: z
    .string()
    .optional()
...
export const askGeminiTool = {
  name: "ask-gemini",
  description:
    "Execute Google Gemini with comprehensive parameter support for code analysis, generation, and assistance",
  zodSchema: askGeminiArgsSchema,
  prompt: {
    description:
      "Execute Gemini AI agent for code analysis, generation, debugging, and assistance with full parameter control",
  },
  category: "gemini",
  execute: async (
    args: AskGeminiArgs,
    ...
    try {
      ...
      if (onProgress) {
        onProgress(
          `Executing Gemini with ${modelName} in ${approvalMode || "default"} mode...`
        );
      }
      ...
      const formattedResponse = formatGeminiResponseForMCP(
        result,
        includeMetadata
      );
