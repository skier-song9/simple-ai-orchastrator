...
import { ERROR_MESSAGES, MODELS, SANDBOX_MODES, } from "../constants.js";
/*
...
const askCodexArgsSchema = z.object({
    prompt: z
        .string()
        .min(1)
        .describe("User query or instruction for Codex. Can include file references and complex requests."),
    model: z
        .string()
        .optional()
        .describe(`Optional model to use. Options: ${Object.values(MODELS).join(", ")}. Defaults to ${MODELS.GPT_5}.`),
    sandbox: z
        .string()
        .optional()
        .describe(`Sandbox mode: ${Object.values(SANDBOX_MODES).join(", ")}. Defaults to read-only for safety.`),
    fullAuto: z
        .boolean()
        .optional()
        .describe("Enable full-auto mode: auto-approve commands with workspace-write sandbox. Convenient for trusted operations."),
    workingDir: z
        .string()
        .optional()
...
export const askCodexTool = {
    ...
    execute: async (args, onProgress) => {
        const { prompt, model, sandbox, fullAuto, workingDir, timeout, image, config, profile, includeMetadata, } = args;
        if (!prompt?.trim()) {
        ...
        try {
            ...
            if (onProgress) {
                onProgress(`Executing Codex with ${modelName}${fullAuto ? " (full-auto)" : ""}...`);
            }
            const result = await executeCodex(prompt.trim(), {
                model,
                sandbox,
                fullAuto,
                workingDir,
            ...
            const formattedResponse = formatCodexResponseForMCP(result, includeMetadata);
