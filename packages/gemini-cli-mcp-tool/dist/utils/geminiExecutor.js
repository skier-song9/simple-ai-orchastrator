...
import { parseGeminiOutput, formatGeminiResponse, } from "./outputParser.js";
import { Logger } from "./logger.js";
import { CLI, APPROVAL_MODES, OUTPUT_FORMATS, ERROR_MESSAGES, } from "../constants.js";
/**
 ...
 */
export async function executeGemini(prompt, options = {}, onProgress) {
    const { model, approvalMode, yolo, sandbox, outputFormat = OUTPUT_FORMATS.TEXT, workingDir, timeout, includeDirectories, debug, } = options;
    ...
    Logger.sandboxMode(sandbox ? "sandbox" : approvalMode || APPROVAL_MODES.DEFAULT, `${CLI.COMMANDS.GEMINI} ${args.join(" ")}`);
    try {
        const rawOutput = await executeCommand(CLI.COMMANDS.GEMINI, args, onProgress, timeout, workingDir);
        ...
        if (approvalMode)
            parsedOutput.metadata.approvalMode = approvalMode;
        if (workingDir)
            parsedOutput.metadata.workingDir = workingDir;
        return parsedOutput;
    }
    catch (error) {
 ...
 */
export function formatGeminiResponseForMCP(output, includeMetadata = true) {
