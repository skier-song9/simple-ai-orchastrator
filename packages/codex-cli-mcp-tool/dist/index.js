    ...
    state.interval = setInterval(async () => {
        if (state.isProcessing && state.progressToken) {
            ...
            const message = outputPreview
                ...
                : baseMessage;
            await sendProgressNotification(state.progressToken, progress, undefined, message);
            messageIndex++;
        }
...
function stopProgressUpdates(state, success = true) {
    state.isProcessing = false;
    clearInterval(state.interval);
    // Send final progress notification if client requested progress
    if (state.progressToken) {
        sendProgressNotification(state.progressToken, 100, 100, success
